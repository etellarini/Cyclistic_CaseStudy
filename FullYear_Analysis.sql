/*
Cyclistic (Case Study): Full year (2022_Q2 - 2023_Q1) - Data Analysis
...
FROM
	...
	FROM
		[Cyclistic].[dbo].[TripData_Integrated12m_StationsAdd]
	) t


-- Total Trips by day: Member vs Casual
...
SELECT 
    CONVERT(TIME, DATEADD(SECOND, AVG(DATEPART(SECOND, [ride_length])) + AVG(DATEPART(MINUTE, [ride_length])) * 60 
		+ AVG(DATEPART(HOUR, [ride_length])) * 3600, 0)) AS [Avg RideLength_Overall],
    CONVERT(TIME, DATEADD(SECOND, AVG(CASE WHEN [member_casual] = 'member' THEN DATEPART(SECOND, [ride_length]) ELSE 0 END)
        + AVG(CASE WHEN [member_casual] = 'member' THEN DATEPART(MINUTE, [ride_length]) 
        ELSE 0 END) * 60 + AVG(CASE WHEN [member_casual] = 'member' THEN DATEPART(HOUR, [ride_length]) 
        ELSE 0 END) * 3600, 0)) AS [Avg RideLength_Member],
    CONVERT(TIME, DATEADD(SECOND, AVG(CASE WHEN [member_casual] = 'casual' THEN DATEPART(SECOND, [ride_length]) ELSE 0 END)
        + AVG(CASE WHEN [member_casual] = 'casual' THEN DATEPART(MINUTE, [ride_length]) 
        ELSE 0 END) * 60 + AVG(CASE WHEN [member_casual] = 'casual' THEN DATEPART(HOUR, [ride_length]) 
        ELSE 0 END) * 3600, 0)) AS [Avg RideLength_Casual]
FROM 
    [Cyclistic].[dbo].[TripData_Integrated12m_StationsAdd];


-- Time range of trips  (Overall and per member type)
...
-- Looking at Median ride length per day of week for member and casual
...
-- Overall trips per day
 -- Looking at total number of trips per day_name
...
SELECT 
    rideable_type,
    CONVERT(TIME, DATEADD(SECOND, CAST(AVG(CAST(DATEDIFF(SECOND, '00:00:00', ride_length) AS BIGINT)) AS INT), '00:00:00')) AS AVG_Ride_Length
FROM 
    [Cyclistic].[dbo].[TripData_Integrated12m_StationsAdd]
GROUP BY 
    rideable_type;
...
-- Looking at Toptal Trips per by bike type
...
-- End stations: member vs casual
 ...
 SELECT 
    ...
    SUM(CASE WHEN ride_id = ride_id AND end_station_name = end_station_name THEN 1 ELSE 0 END) AS total,
...
SELECT 
    DATEPART(HOUR, started_hour) AS hour_of_day, 
    COUNT(*) AS trip_count 
FROM 
    [Cyclistic].[dbo].[TripData_Integrated12m_StationsAdd]
WHERE 
